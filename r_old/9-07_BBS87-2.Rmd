...
df_pivot[nrow(df_pivot) + 1,] = c(20, 0, 0, 0, 0, 0)
df_pivot[nrow(df_pivot) + 1,] = c(40, 0, 0, 0, 0, 0)

df_pivot <- df_pivot[order(df_pivot$Tiefe),]
...
ggplot(df_melt, aes(rev(factor(Tiefe)), value, fill = factor(variable))) + 
    ...
    scale_y_continuous("Gewicht [g]", expand = c(0, 0), limits = c(0, 770)) +
...
ggsave("../output/figs/9-7_BBS87-2_VerteilungFunde_R.pdf", width = 10, height = 4.3)
```
...
df_pivot[nrow(df_pivot) + 1,] = c(20, 0, 0, 0, 0, 0, 0, 0, 0, 0)
df_pivot[nrow(df_pivot) + 1,] = c(40, 0, 0, 0, 0, 0, 0, 0, 0, 0)
...
ggplot(df_melt, aes(rev(factor(Tiefe)), value, fill = factor(variable))) + 
    ...
    coord_flip() + 
    scale_x_discrete("Tiefe [cm u. NP]", labels = c(rev(df_melt$Tiefe))) + 
    scale_y_continuous("Gewicht [g]", expand = c(0, 0), limits = c(0, 605)) +
    ...
    theme(legend.justification = c(1, 1), 
          legend.position = c(.99, .99), 
          panel.grid.major.y = element_blank())
ggsave("../output/figs/9-7_BBS87-2_Fabrics_R.pdf", width = 10, height = 4.3)
```
...
df_pivot <- data.frame(dcast(df, Tiefe ~ Typ, 
                             ...
                             fun.aggregate = sum, 
                             subset = .(!is.na(Typ))))
df_pivot[nrow(df_pivot) + 1,] = c(20, 0, 0, 0)
df_pivot[nrow(df_pivot) + 1,] = c(40, 0, 0, 0)
...
ggplot(df_melt, aes(rev(factor(Tiefe)), 
    ...
    coord_flip() + 
    scale_x_discrete("Tiefe [cm u. NP]", labels = c(rev(df_melt$Tiefe))) + 
    scale_y_continuous("Gewicht [g]", expand = c(0, 0), limits = c(0, 570)) +
    ...
    theme(legend.justification = c(1, 1), 
          legend.position = c(.99, .99), 
          panel.grid.major.y = element_blank())
ggsave("../output/figs/9-7_BBS87-2_KeramikStilgruppen_R.pdf", width = 10, height = 4.3)
```
