---
title: "3.1.2.1 Pikudna-Munda-Gruppe"
...
df1 = dbGetQuery(con, "SELECT t_Obj.objID, t_Ort.ort_kurz, t_Komplex.bef_nr, t_Obj.Individuum, t_Obj.Typ, [t_Ort].[Kat-Nr], t_Obj.Typ, t_Ort.y_lat, t_Ort.x_long 
                 FROM (t_Ort INNER JOIN t_Komplex ON t_Ort.ortID = t_Komplex.ortID) INNER JOIN t_Obj ON t_Komplex.komplexID = t_Obj.komplexID 
                 WHERE (((t_Obj.Typ) = 'PIKMUN')
                    AND ((t_Ort.ort_lit) = 'DS'))")
head(df1)

# nur nicht sicher

df2 = dbGetQuery(con, "SELECT t_Obj.objID, t_Ort.ort_kurz, t_Komplex.bef_nr, t_Obj.Individuum, t_Obj.Typ, [t_Ort].[Kat-Nr], t_Obj.Typ, t_Ort.y_lat, t_Ort.x_long 
                 FROM (t_Ort INNER JOIN t_Komplex ON t_Ort.ortID = t_Komplex.ortID) INNER JOIN t_Obj ON t_Komplex.komplexID = t_Obj.komplexID 
                 WHERE (((t_Obj.Typ) Like '%PIK%?%')
                    AND ((t_Ort.ort_lit) = 'DS'))")
head(df2)
...
ggsave("../output/figs/3-1-2-1_Pikunda-Munda_Verbreitung.pdf", width = 11, height = 11)
...
# Karte
pdf("../output/figs/3-1-2-1_Pikunda-Munda_Verbreitung_C.pdf", width = 8, height = 8)
plotRGB(r)
