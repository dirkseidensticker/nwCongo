...
df1 = dbGetQuery(con, "SELECT t_Obj.objID, t_Obj.ort_kurz, t_Obj.Komplex, t_Obj.Individuum, t_Obj.Typ, [t_Ort].[Kat-Nr], t_Obj.Typ, t_Ort.y_lat, t_Ort.x_long 
                 ...
                 WHERE (((t_Ort.ort_lit)='DS') AND ((t_Obj.Typ) = 'KON'))")
head(df1)
...
df2 = dbGetQuery(con, "SELECT t_Obj.objID, t_Obj.ort_kurz, t_Obj.Komplex, t_Obj.Individuum, t_Obj.Typ, [t_Ort].[Kat-Nr], t_Obj.Typ, t_Ort.y_lat, t_Ort.x_long 
                 FROM (t_Ort INNER JOIN t_Komplex ON t_Ort.ortID = t_Komplex.ortID) INNER JOIN t_Obj ON t_Komplex.komplexID = t_Obj.komplexID 
                 WHERE (((t_Ort.ort_lit)='DS') AND ((t_Obj.Typ) Like '%KON%?%'))")
head(df2)

qmap("Dongou", zoom = 7, color = 'bw', maptype = 'roadmap') + 
  geom_point(aes(x = x_long, y = y_lat), data = df2, size = 4, shape = 21) + 
  geom_text(aes(x = x_long+0.15, y = y_lat+0.07, label = ort_kurz), data = df2, size = 2) + 
  geom_point(aes(x = x_long, y = y_lat), data = df1, size = 5, shape = 21, fill = 'black', color = 'white') + 
...
ggsave("../output/figs/3-1-2-8_Konda_Verbreitung.pdf", width = 11, height = 11)
...
#  geom_point(aes(x = x_long, y = y_lat), data = df1, size = 5, shape = 21, fill = 'black', color = 'white') + 
  ...
  theme(legend.justification = c(0,1), 
        ...
        axis.title.x = element_blank(),
        axis.title.y = element_blank())
ggsave("../output/figs/3-1-2-8_Konda_Verbreitung_B.pdf", width = 11, height = 11)
...
# Karte
pdf("../output/figs/3-1-2-8_Konda_Verbreitung_C.pdf", width = 8, height = 8)
plotRGB(r)
