##################################
# 3.1.1.1 Batalimo-Maluba-Gruppe #
##################################
...
# nur sicher
df1 = dbGetQuery(con, "SELECT t_Obj.objID, t_Obj.ort_kurz, t_Obj.Komplex, t_Obj.Individuum, t_Obj.Typ, [t_Ort].[Kat-Nr], t_Ort.y_lat, t_Ort.x_long
                 FROM (t_Ort INNER JOIN t_Komplex ON t_Ort.ortID = t_Komplex.ortID) INNER JOIN t_Obj ON t_Komplex.komplexID = t_Obj.komplexID
                 WHERE (((t_Ort.ort_lit)='DS') AND ((t_Obj.Typ) = 'BatMLB'))")
head(df1)

# nur nicht sicher
df2 = dbGetQuery(con, "SELECT t_Obj.objID, t_Obj.ort_kurz, t_Obj.Komplex, t_Obj.Individuum, t_Obj.Typ, [t_Ort].[Kat-Nr], t_Ort.y_lat, t_Ort.x_long
                 FROM (t_Ort INNER JOIN t_Komplex ON t_Ort.ortID = t_Komplex.ortID) INNER JOIN t_Obj ON t_Komplex.komplexID = t_Obj.komplexID
                 WHERE (((t_Ort.ort_lit)='DS') AND ((t_Obj.Typ) Like '%Bat%?%'))")
head(df2)

df_lit = dbGetQuery(con, "SELECT t_Ort.ort_name, t_Ort.y_lat, t_Ort.x_long
                FROM t_Ort
                WHERE ((t_Ort.ort_name) = 'Batalimo')")
head(df_lit)
...
qmap("Dongou", zoom = 7, color = 'bw', maptype = 'roadmap') + 
...
ggsave("../output/figs/3-1-1-1_Batalimo-Maluba_Verbreitung.pdf", width = 11, height = 11)
...
ggsave("../output/figs/3-1-1-1_Batalimo-Maluba_Verbreitung_B.pdf", width = 11, height = 11)
...
qmap("Dongou", zoom = 7, maptype = 'roadmap', color = 'bw') + 
  ...
  theme(legend.justification = c(0,1), 
      ...
      axis.title.x = element_blank(),
      axis.title.y = element_blank())
ggsave("../output/figs/3-1-1-1_Batalimo-Maluba_Verbreitung_C.pdf", width = 11, height = 11)
...
locs3 <- subset(df_lit, select = c("ort_name", "y_lat", "x_long"))
coordinates(locs3) <- c("x_long", "y_lat")  # set spatial coordinates
...
# Karte
pdf("../output/figs/3-1-1-1_Batalimo-Maluba_Verbreitung_D.pdf", width = 8, height = 8)
plotRGB(r)
...
text(x = locs3$x_long, y = locs3$y_lat, labels=locs3$ort_name, pos=2, cex=0.8)
